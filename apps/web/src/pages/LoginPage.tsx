...
export function LoginPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : 'No fue posible continuar.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="auth-page">
      <section className="auth-showcase">
        ...
        <div className="feature-grid">
          <div><BarChart3 /><span><strong>Decisiones claras</strong><small>Indicadores en tiempo real</small></span></div>
          <div><FolderKanban /><span><strong>Proyectos bajo control</strong><small>Tareas, fechas y progreso</small></span></div>
          <div><ShieldCheck /><span><strong>Datos protegidos</strong><small>Acceso por empresa y rol</small></span></div>
          <div><CheckCircle2 /><span><strong>Flujos conectados</strong><small>Del prospecto al proyecto</small></span></div>
        </div>
      </section>
      ...
      <section className="auth-form-panel">
        <form className="auth-form" onSubmit={submit}>
          <div>
            <p className="eyebrow">{mode === 'login' ? 'Bienvenido de nuevo' : 'Comienza hoy'}</p>
            <h2>{mode === 'login' ? 'Inicia sesión' : 'Crea tu espacio de trabajo'}</h2>
            <p>
              {mode === 'login'
          ...
          {mode === 'register' && (
            <>
              <label className="form-field full">
                <span>Empresa</span>
                <input name="companyName" placeholder="Nombre de la empresa" required />
              </label>
              <label className="form-field full">
