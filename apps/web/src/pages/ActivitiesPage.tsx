        ...
        <section className="page calendar-page">
          <div className="page-heading calendar-heading">
            <div>
              ...
              <p>Visualiza reuniones, llamadas, visitas y seguimientos por día.</p>
            </div>
            <div className="calendar-controls">
              <button aria-label="Mes anterior" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))} type="button">
                <ChevronLeft size={18} />
              </button>
              <strong>{new Intl.DateTimeFormat('es-DO', { month: 'long', year: 'numeric' }).format(month)}</strong>
              <button aria-label="Mes siguiente" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))} type="button">
                <ChevronRight size={18} />
              </button>
              <button className="secondary-button" onClick={() => setMonth(new Date(new Date().getFullYear(), new Date().getMonth(), 1))} type="button">
                Hoy
              </button>
            </div>
          </div>
              ...
              <article className={currentMonth ? 'calendar-day' : 'calendar-day muted'} key={date.toISOString()}>
                ...
                <div>
                  {dayItems.slice(0, 4).map((item) => (
                    <span className={`calendar-event calendar-event--${item.type.toLowerCase()}`} key={item.id} title={`${item.subject} · ${item.customer?.name ?? ''}`}>
                      <b>{new Intl.DateTimeFormat('es-DO', { hour: 'numeric', minute: '2-digit' }).format(new Date(item.scheduledAt))}</b>
                      {item.subject}
                    </span>
                  ))}
