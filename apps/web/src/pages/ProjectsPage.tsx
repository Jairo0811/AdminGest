...
export function ProjectsPage() {
  ...
  const taskTimeline = useMemo(() => {
    const tasks = (selectedProject.data?.tasks ?? []).filter((task) => task.startDate && task.dueDate);
    if (!tasks.length) return null;
    const min = Math.min(...tasks.map((task) => new Date(task.startDate!).getTime()));
    const max = Math.max(...tasks.map((task) => new Date(task.dueDate!).getTime()));
    return { tasks, min, max, span: Math.max(86_400_000, max - min) };
  ...
  const toCsvRows = (project: Project): ProjectCsvRow[] => {
    ...
    return tasks.map((task, index) => ({
      ...
      resourceNames: task.assignee
        ...
        : '',
      predecessors: task.parentId && idMap.has(task.parentId)
        ? [idMap.get(task.parentId)!]
        : [],
      milestone: Boolean(task.startDate && task.dueDate && daysBetween(task.startDate, task.dueDate) === 0),
    }));
  };
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const descriptionParts = [
          row.resourceNames ? `Recursos de Microsoft Project: ${row.resourceNames}` : '',
          row.milestone ? 'Hito importado desde Microsoft Project.' : '',
        ].filter(Boolean);

        const created = await api<ProjectTask>(`/projects/${selectedProjectId}/tasks`, {
          method: 'POST',
          body: JSON.stringify({
            title: row.name,
        ...
        if (row.percentComplete > 0) {
          await api(`/projects/tasks/${created.id}`, {
            ...
            body: JSON.stringify({
              ...
              status: row.percentComplete >= 100 ? 'COMPLETED' : 'IN_PROGRESS',
            }),
          });
        }
      }
      ...
      await queryClient.invalidateQueries({ queryKey: ['/projects', selectedProjectId] });
  ...
  const printTaskGantt = () => {
    ...
    const rows = taskTimeline.tasks.map((task) => {
      const start = new Date(task.startDate!).getTime();
      const end = new Date(task.dueDate!).getTime();
      const left = ((start - taskTimeline.min) / taskTimeline.span) * 100;
      const width = Math.max(2, ((end - start) / taskTimeline.span) * 100);
      const late = task.status !== 'COMPLETED' && new Date(task.dueDate!) < new Date();
        ...
        <section className="page timeline-page">
          ...
          {projects.isLoading ? <div className="board-skeleton">Cargando cronograma…</div> : !timeline ? (
            ...
            <div className="timeline-card">
              <div className="timeline-scale"><span>{new Date(timeline.min).toLocaleDateString('es-DO')}</span><span>{new Date((timeline.min + timeline.max) / 2).toLocaleDateString('es-DO')}</span><span>{new Date(timeline.max).toLocaleDateString('es-DO')}</span></div>
              <div className="timeline-list">
                {timeline.projects.map((project) => {
                  ...
                  return <article className="timeline-row" key={project.id}><div className="timeline-project"><strong>{project.name}</strong><small>{project.customer.name} · {project.status}</small></div><div className="timeline-track"><div className="timeline-bar" style={{ left: `${left}%`, width: `${width}%` }}><i style={{ width: `${project.progress}%` }} /><span>{project.progress}%</span></div></div></article>;
                })}
              </div>
            </div>
          )}
        ...
        <section className="page ms-project-page">
          ...
          <div className="ms-project-toolbar panel">
            ...
            <div className="ms-project-actions">
              <label className={selectedProjectId && !importing ? 'secondary-button file-button' : 'secondary-button file-button disabled'}>
                <FileUp size={17} /> {importing ? 'Importando…' : 'Importar CSV'}
                <input accept=".csv,text/csv" disabled={!selectedProjectId || importing} onChange={(event) => void importCsv(event)} type="file" />
              </label>
              <button className="secondary-button" disabled={!selectedProject.data?.tasks?.length} onClick={exportCsv} type="button"><Download size={17} /> Exportar CSV</button>
              <button className="secondary-button" disabled={!taskTimeline} onClick={printTaskGantt} type="button"><Printer size={17} /> Imprimir Gantt</button>
            </div>
          </div>
          ...
          {!selectedProjectId ? (
            <div className="empty-state panel"><strong>Selecciona un proyecto.</strong><p>Después podrás importar tareas desde Microsoft Project o exportarlas.</p></div>
          ) : selectedProject.isLoading ? (
            <div className="board-skeleton">Cargando tareas…</div>
          ) : !taskTimeline ? (
            <div className="empty-state panel"><strong>No hay tareas con fechas completas.</strong><p>Importa un CSV o registra inicio y fin en las tareas.</p></div>
          ) : (
            <div className="timeline-card task-gantt">
              <div className="timeline-scale"><span>{new Date(taskTimeline.min).toLocaleDateString('es-DO')}</span><span>{new Date((taskTimeline.min + taskTimeline.max) / 2).toLocaleDateString('es-DO')}</span><span>{new Date(taskTimeline.max).toLocaleDateString('es-DO')}</span></div>
              <div className="timeline-list">
                {taskTimeline.tasks.map((task) => {
                  ...
                  return <article className={late ? 'timeline-row task-late' : 'timeline-row'} key={task.id}><div className="timeline-project"><strong>{milestone && <Milestone size={14} />} {task.title}</strong><small>{task.status} · {task.progress}%{late ? ' · Atrasada' : ''}</small></div><div className="timeline-track"><div className={milestone ? 'timeline-bar timeline-milestone' : 'timeline-bar'} style={{ left: `${left}%`, width: `${width}%` }}><i style={{ width: `${task.progress}%` }} /><span>{milestone ? '◆' : `${task.progress}%`}</span></div></div></article>;
                })}
              </div>
            </div>
          )}
