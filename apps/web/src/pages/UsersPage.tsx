...
import { Edit3, Plus, ShieldCheck, UserCheck, UserX } from 'lucide-react';
...
export function UsersPage() {
  ...
  const toggleStatus = useMutation({
    mutationFn: (item: UserItem) =>
      api<UserItem>(`/users/${item.id}`, {
        method: 'PATCH',
    ...
    <section className="page">
      ...
      <div className="page-heading">
        <div>
          ...
          <p>Gestiona el acceso del equipo y aplica el principio de mínimo privilegio.</p>
        </div>
      ...
      <div className="toolbar entity-toolbar">
        ...
        <span className="record-count">{filtered.length} usuarios</span>
      </div>
      ...
      <div className="table-card">
        <div className="data-table table-header">
          <strong>Usuario</strong><strong>Rol</strong><strong>Estado</strong><strong>Último acceso</strong><strong>Acciones</strong>
        </div>
        {usersQuery.isLoading ? (
          <div className="table-skeleton" aria-label="Cargando usuarios">{Array.from({ length: 4 }, (_, index) => <span key={index} />)}</div>
        ) : filtered.length === 0 ? (
          <div className="empty-state"><strong>No hay usuarios</strong><p>Crea el primer integrante del equipo.</p></div>
        ) : filtered.map((item) => (
          <div className="data-table table-row" key={item.id}>
            ...
            <div data-label="Estado"><span className={`status-badge ${item.status === 'ACTIVE' ? 'green' : 'gray'}`}>{item.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}</span></div>
            ...
            <div className="row-actions" data-label="Acciones">
              ...
              <button
                aria-label={item.status === 'ACTIVE' ? 'Desactivar usuario' : 'Activar usuario'}
                className={`table-action ${item.status === 'ACTIVE' ? 'danger' : ''}`}
                disabled={item.id === sessionUser?.id || toggleStatus.isPending}
                onClick={() => toggleStatus.mutate(item)}
                title={item.id === sessionUser?.id ? 'No puedes desactivar tu propia cuenta.' : undefined}
                type="button"
              >
                {item.status === 'ACTIVE' ? <UserX size={17} /> : <UserCheck size={17} />}
              </button>
            </div>
          </div>
        ))}
      ...
      {editing !== undefined && (
        <Modal onClose={() => { setEditing(undefined); setError(''); }} title={editing ? 'Editar usuario' : 'Nuevo usuario'}>
