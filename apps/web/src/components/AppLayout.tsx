...
import {
  BarChart3, Bell, Boxes, CalendarDays, ChevronDown, ContactRound,
  FileText, FolderKanban, LayoutDashboard, LogOut, Menu, Moon,
  PanelLeftClose, PanelLeftOpen, Search, Settings, Sun, Target,
  UserCircle, UserCog, UsersRound, X,
} from "lucide-react";
...
export function AppLayout() {
  ...
  const searchQuery = useQuery({
    ...
    queryFn: () => api<SearchResult[]>(`/navigation/search?q=${encodeURIComponent(search.trim())}`),
  ...
  const currentLabel = useMemo(() => {
    ...
    return navigation.find((item) => item.to === "/" ? location.pathname === "/" : location.pathname.startsWith(item.to))?.label ?? "AdminGest";
  }, [location.pathname]);
  ...
  const goTo = (path: string) => {
    navigate(path); setSearch(""); setNotificationsOpen(false); setProfileOpen(false); setMenuOpen(false);
  };

  return (
    <div className={collapsed ? "app-shell app-shell--collapsed" : "app-shell"}>
      <aside className={["sidebar", menuOpen ? "open" : "", collapsed ? "sidebar--collapsed" : ""].filter(Boolean).join(" ")}>
        <div className="brand-row"><div className="brand"><BrandLogo compact={collapsed} variant="sidebar" /></div><button aria-label="Cerrar menú" className="mobile-close" onClick={() => setMenuOpen(false)} type="button"><X size={20} /></button></div>
        <div className="company-chip" title={user?.company.name}><small>Empresa activa</small><strong>{user?.company.name}</strong></div>
        <nav className="sidebar-nav" aria-label="Navegación principal">
          {navigation.map(({ icon: Icon, label, to }) => <NavLink aria-label={label} className={({ isActive }) => isActive ? "nav-item active" : "nav-item"} end={to === "/"} key={to} onClick={() => setMenuOpen(false)} title={collapsed ? label : undefined} to={to}><Icon size={19} /><span>{label}</span></NavLink>)}
          {isAdmin && <NavLink className={({ isActive }) => isActive ? "nav-item active" : "nav-item"} to="/users"><UserCog size={19} /><span>Usuarios</span></NavLink>}
        </nav>
        <NavLink className="nav-item settings" to="/settings"><Settings size={19} /><span>Configuración</span></NavLink>
        <button className="nav-item logout" onClick={logout} type="button"><LogOut size={19} /><span>Cerrar sesión</span></button>
      </aside>
      ...
      <div className="workspace-shell">
        <header className="topbar">
          ...
          <button aria-label={collapsed ? "Expandir barra lateral" : "Colapsar barra lateral"} className="sidebar-toggle" onClick={toggleSidebar} type="button">{collapsed ? <PanelLeftOpen size={19} /> : <PanelLeftClose size={19} />}</button>
          <div className="breadcrumbs"><span>AdminGest</span><b>/</b><strong>{currentLabel}</strong></div>
          <div className="global-search"><Search size={18} /><input aria-label="Buscar en AdminGest" onChange={(event) => setSearch(event.target.value)} placeholder="Buscar clientes, proyectos..." value={search} />
            {search.trim().length >= 2 && <div className="search-results">{searchQuery.isLoading ? <div className="popover-state">Buscando…</div> : searchQuery.data?.length ? searchQuery.data.map((item) => <button key={`${item.type}-${item.id}`} onClick={() => goTo(item.path)} type="button"><span className="search-result-type">{item.type}</span><span><strong>{item.title}</strong><small>{item.subtitle}</small></span></button>) : <div className="popover-state">No encontramos coincidencias.</div>}</div>}
          </div>
          <div className="topbar-spacer" />
          <button aria-label={theme === "dark" ? "Activar tema claro" : "Activar tema oscuro"} className="icon-button" onClick={toggleTheme} type="button">{theme === "dark" ? <Sun size={20} /> : <Moon size={20} />}</button>
