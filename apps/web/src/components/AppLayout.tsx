    ...
    <div className={collapsed ? "app-shell app-shell--collapsed" : "app-shell"}>
      <aside className={["sidebar", menuOpen ? "open" : "", collapsed ? "sidebar--collapsed" : ""].filter(Boolean).join(" ")}>
        <div className="brand-row">
          <div className="brand"><BrandLogo compact={collapsed} variant="sidebar" /></div>
          <button aria-label="Cerrar menú" className="mobile-close" onClick={() => setMenuOpen(false)} type="button"><X size={20} /></button>
        </div>
        ...
        <nav className="sidebar-nav" aria-label="Navegación principal">
          {navigation.map(({ icon: Icon, label, to }) => (
            <NavLink aria-label={label} className={({ isActive }) => isActive ? "nav-item active" : "nav-item"} end={to === "/"} key={to} onClick={() => setMenuOpen(false)} title={collapsed ? label : undefined} to={to}>
        ...
        <header className="topbar">
              ...
              <div className="search-results">
                {searchQuery.isLoading ? <div className="popover-state">Buscando…</div> : searchQuery.data?.length ? searchQuery.data.map((item) => (
                  <button key={`${item.type}-${item.id}`} onClick={() => goTo(item.path)} type="button">
                    <span className="search-result-type">{item.type}</span><span><strong>{item.title}</strong><small>{item.subtitle}</small></span>
                  </button>
          ...
          <div className="topbar-popover-anchor">
            <button aria-label="Notificaciones" className="icon-button" onClick={() => { setNotificationsOpen((value) => !value); setProfileOpen(false); }} type="button">
              <Bell size={20} />{!!notificationsQuery.data?.length && <span className="notification-dot" />}
            </button>
            {notificationsOpen && (
              <div className="topbar-popover notifications-popover">
                ...
                {notificationsQuery.isLoading ? <div className="popover-state">Cargando…</div> : notificationsQuery.data?.length ? notificationsQuery.data.map((item) => (
                  <button key={item.id} onClick={() => goTo(item.path)} type="button"><span><strong>{item.title}</strong><small>{item.description}</small><time>{new Date(item.occurredAt).toLocaleString("es-DO")}</time></span></button>
          ...
          <div className="topbar-popover-anchor">
            <button className="user-summary user-summary--button" onClick={() => { setProfileOpen((value) => !value); setNotificationsOpen(false); }} type="button">
              ...
              <div><strong>{user?.firstName} {user?.lastName}</strong><small>{user?.role.replaceAll("_", " ")}</small></div>
              <ChevronDown size={16} />
            </button>
            {profileOpen && (
              <div className="topbar-popover profile-popover">
                <button onClick={() => goTo("/settings")} type="button"><UserCircle size={18} /> Mi perfil y empresa</button>
